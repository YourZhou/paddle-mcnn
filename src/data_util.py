...
def read_train_data2(img_path, gt_path, crop_size=256, scale=4):
    ...
    # 修剪和缩放后，生成需要尺寸的密度图（64x64）
    k = np.zeros((cropped_scaled_crowd_img.shape[0], cropped_scaled_crowd_img.shape[1]))
    gt = cropped_scaled_points
    # 生成稀疏矩阵的热编码矩阵
    for i in range(0, len(gt)):
        if int(gt[i][1]) < cropped_scaled_crowd_img.shape[0] and int(gt[i][0]) < cropped_scaled_crowd_img.shape[1]:
